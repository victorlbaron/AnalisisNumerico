...
#impresion base
plot(f,xlim = c(-4,4), ylim=c(-1.5,1.5), ylab="sin(x)", col = "orange")
abline(0,0, col = "gray")
abline(v=0, col = "gray")
...
par(new = TRUE)
plot(final, xlim = c(-4,4), ylim = c(-1.5,1.5), col = "cyan4", ylab= "")
par(new = TRUE)
plot(xo,f(xo), xlim = c(-4,4), ylim = c(-1.5,1.5), col = "red", ylab= "")

#mestra del error en dos puntos
error = abs(f(pi/4)-final(pi/4))
print(error)
